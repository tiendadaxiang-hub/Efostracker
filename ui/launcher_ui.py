...
try:
    ...
    from core.notifier import enviar_reporte, _smtp_configurado
    from core.parser import cargar_procesado
...
"""
    ...
    def _worker_query_single(self, rfc):
        try:
            ...
            # Validación robusta para evitar el error NoneType
            if res is None:
                output = "Error: No se pudo obtener respuesta del backend. Verifica que existan datos procesados."
            else:
                output = self._format_result_table([res])
            self.root.after(0, self._set_results_text, output)
        except Exception as e:
            self.root.after(0, self._set_results_text, f"Error inesperado: {e}")
    ...
    def _worker_query_csv(self, path_str):
        try:
            ...
            if not resultados:
                output = "No se encontraron RFCs válidos en el archivo CSV."
            else:
    ...
    def _update_status_labels(self):
        try:
            ...
            if latest_path.exists():
                ...
                self.lbl_last_date.config(text="Último listado: Sin datos (Ejecuta un ciclo primero)")
        except: pass
    ...
    def _format_result_table(resultados: list[dict]) -> str:
        ...
        if not resultados:
            return "No se encontraron resultados o la lista está vacía."
        ...
        rows = []
        for r in resultados:
            # Protección contra None o diccionarios malformados
            ...
            nombre = r.get('nombre', 'N/A')[:50] if r.get('nombre') else 'N/A'
            
            rows.append(f"{rfc:<15} | {encontrado:<12} | {situacion:<15} | {nombre}")
            
        if not rows:
            return "No se pudieron formatear los resultados."
